  ...
  at::parallel_for(0, MB2 * NB2, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; ++i) {
      ...
      for (int64_t m = 0; m < m_size; ++m) {
        ...
        copy_mul_stub<scalar_t, scalar_t>(ic2 + index * K + nb * BLOCK_N, C + m * BLOCK_N, weight, n_size);
      }
    }
...
void shared_expert_fp8_kernel_impl(
  ...
  at::parallel_for(0, MB2 * NB2, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; ++i) {
      ...
      for (int64_t m = 0; m < m_size; ++m) {
        add_mul_stub<scalar_t, scalar_t>(out + m * K, C + m * BLOCK_N,
                                         fused_out ? fused_out + m * K : nullptr,
